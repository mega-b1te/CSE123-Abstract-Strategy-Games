...
public class Testing {
    ...
    @Test
    @DisplayName("Small Connect Four Example")
    public void firstConnectFourCaseTest() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            // -1 is an illegal move so our code should throw an IllegalArgumentException
            g.makeMove(new Scanner("A -1"));
        }, "IllegalArgumentException not thrown for illegal move");
    }

    @Test
    @DisplayName("Large Connect Four Example")
    public void secondConnectFourCaseTest() {
        // You definitely don't have to get this fancy in your tests!
        AbstractStrategyGame g = new ConnectFour();

        // Going to play a whole game where 1 plays in bottom row, 2 plays in second row
        // No optional error messages - up to you if you want your code to be easier to debug!
        for (int i = 0; i < 4; i++) {
        ...
        // At this point, 7 moves have been played, player 1 should have four in a row and
        // player 2 should have three
        assertTrue(g.isGameOver());
        assertEquals(1, g.getWinner());
        assertEquals(-1, g.getNextPlayer());
    }
